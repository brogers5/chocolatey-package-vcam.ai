...
function Uninstall-CurrentVersion {
    ...
    elseif ($keys.Count -le 2) {
        #VCam.ai adds a couple keys. We need to use the one named after the product code GUID.
...
function Get-PredecessorUninstallRegistryKey {
    return [array] (Get-UninstallRegistryKey -SoftwareName 'XSplit VCam')
}
